...
def index():
    ...
    if request.method == 'POST':
        city_q = request.form.get('city', '').strip()
        if city_q:
            try:
                ...
                if lat is None:
                    ...
                    try:
                        city_id = db.upsert_city(
                            display_name, lat, lon, weather['city'], weather['state']
                        )
                        wr_id = db.save_weather_result(city_id, weather)
                        db.save_lookup(
                            city_q, city_id=city_id, weather_result_id=wr_id, success=True
                        )
                    except Exception as e:
                        app.logger.warning('DB write failed: %s', e)
                ...
                if code == 404:
                    ...
                    error = f'Weather API error (HTTP {code}). Please try again.'
                try:
                    db.save_lookup(city_q, success=False, error_message=error)
    ...
    return render_template('index.html', weather=weather, error=error, city_q=city_q, recent=_db_recent())
